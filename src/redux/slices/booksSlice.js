...
export const getBooks = createAsyncThunk("booksSlice/getBooks", 
   async ([searchValue, startIndex], {rejectWithValue}) => {
      try {
         ...
         const response = await fetch(`https://www.googleapis.com/books/v1/volumes?q=${searchValue}&orderBy=relevance&key=${process.env.REACT_APP_GOOGLE_BOOK_API_KEY}&startIndex=${startIndex ? startIndex : 0}&maxResults=30`)
